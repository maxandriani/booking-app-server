...
public class AddBookingGuestCmdHandlerTest : TestBase
{
    ...
    public async Task Should_Throw_ValidationException_When_GuestId_Is_Empty()
    {
        ...
        var booking = new BookingApp.Core.Bookings.Models.Booking() { PlaceId = place.Id, CheckIn = DateTime.UtcNow, CheckOut = DateTime.UtcNow.AddDays(5), Status = Core.Bookings.Models.BookingStatusEnum.Confirmed };
        var dbContext = _injector.GetRequiredService<BookingDbContext>();
        dbContext.Add(place);
